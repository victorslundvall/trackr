...
11. `muscle_feedback_live_prs` – `muscle_feedback` (återhämtning/pump/ansträngning/ledkänning per muskel och pass), `set_adjustments` (föreslagna och godkända set-ändringar) och `exercise_bests(p_exercises, p_exclude_workout)` för rekord live under passet.
12. `coach_feedback_zones_reviews` – `muscle_feedback_history(p_weeks)` (varje feedbacksvar med muskelns hårda set senaste 7 dagarna + dagsform) för personliga volymzoner och coachens kontext, samt `coach_reviews` (coachens andra åsikt efter pass: signaler, sammanfattning, åtgärder).
