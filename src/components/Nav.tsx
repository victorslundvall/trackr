...
import { BarChart3, CalendarDays, Dumbbell, Home, Menu } from "lucide-react";
...
export default function Nav() {
  ...
  if (path.startsWith("/login") || path.startsWith("/workout/")) return null;
  return (
        ...
        {items.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <li key={href}>
              <Link
                href={href}
                className={`flex flex-col items-center gap-1 py-2.5 text-[11px] font-medium ${active ? "text-accent" : "text-ink-3 hover:text-ink-2"}`}
              >
